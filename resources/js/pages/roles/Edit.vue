<template>
    <div class="container mx-auto max-w-7xl p-4">
        <div class="mb-6">
            <h1 class="mb-2 text-3xl font-bold text-gray-900 dark:text-white">Editar Role</h1>
            <p class="text-gray-600 dark:text-gray-400">Atualize as informações básicas e permissões da role</p>
        </div>
...
// Clone modules data and set permissions based on role
const modules = reactive(JSON.parse(JSON.stringify(moduleData)));
...
if (props.role.permissions) {
    modules.forEach(module => {
        module.permissions.forEach(permission => {
            permission.enabled = props.role.permissions!.includes(permission.id);
        });
    });
}
